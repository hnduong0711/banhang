...
//   const userId = "13101c92-7ce0-4202-8f53-a6f3af28b222";
...
function AdminOrders() {
  ...
  // Lấy tất cả hóa đơn và lọc trạng thái PAID
  const getAllInvoices = async () => {
    try {
      const response = await axios.get("http://localhost:5155/api/Invoice", {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
      ...
      const allInvoices = response.data;
      // Lọc chỉ lấy hóa đơn có status là "PAID"
      const successInvoices = allInvoices.filter(
        (invoice) => invoice.status === "PAID"
      );
      setInvoices(successInvoices);
